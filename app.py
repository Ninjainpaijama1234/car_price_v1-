...
def _build_ohe() -> OneHotEncoder:
    # sklearn: some versions use 'sparse_output', older use 'sparse'
...
# Streamlit cache: avoid UnhashableParamError
CACHE_HASH_FUNCS = {
    ...
    TfidfVectorizer: id,
    OneHotEncoder: id,
    HistGradientBoostingRegressor: id,
    RandomForestRegressor: id,
}
...
def train_and_select_model(X: pd.DataFrame, y: np.ndarray, preprocessor: ColumnTransformer):
    ...
    for name, model, param_dist in candidates:
        ...
        # OOF predictions for metrics (AED space)
    ...
    return best, lb, {"X_test": X_test, "y_test": y_test, "abs_residuals": abs_residuals}
...
def sidebar_inputs(df: pd.DataFrame, meta: MetaInfo) -> dict:
    ...
    # safer default median
    year_series = pd.to_numeric(df["Year"], errors="coerce").dropna()
    default_year = int(year_series.median()) if not year_series.empty else 2020
    ...
    mileage_series = pd.to_numeric(df["Mileage"], errors="coerce").dropna()
    mileage_default = int(mileage_series.median()) if not mileage_series.empty else 100000
...
def main():
    ...
    if os.path.exists(BEST_MODEL_PATH) and os.path.exists(CONFORMAL_PATH):
        ...
        with st.spinner("Training models & selecting the best (first-run only)..."):
